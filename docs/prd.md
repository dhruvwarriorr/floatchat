...
1. The system must accept a free-text natural-language query and parse it into a validated structured query (`query_type`, `location`, `parameter`, `date_from`, `date_to`, `include_anomaly`). A configured LLM may propose this structure, but deterministic rules remain authoritative for supported geography, dates, parameters, radius, and intent.
2. The system must fall back to a rule-based parser (canonical place/region gazetteer + regex) if the LLM is disabled, missing, times out, fails, or returns invalid output, and must tag every response with `parser_used: "llm" | "rule_based"`.
...
7. On QC-passed data only, the system must compute an anomaly score (`z = (x − μ) / σ` vs. a precomputed regional/monthly climatology baseline) whenever the production baseline and evidence policy permit it. An explicit anomaly request changes the answer emphasis, not the scientific eligibility gate. The score must be classified as `normal`, `mild_positive`, `mild_negative`, `strong_positive`, or `strong_negative`. Results must be labeled "upper-ocean temperature anomaly" or "salinity anomaly" — never "marine heatwave," since that term requires daily SST above a seasonally varying percentile threshold for a sustained duration, a different method than sparse-profile Z-scoring.
